...
pub fn run(input: String) -> Result<(), AoCError> {
    ...
    let total_size = root.borrow().dir_sizes(&mut sizes);

    let free_space = 70000000 - total_size;
    let space_required = 30000000 - free_space;

    println!("{} {}", free_space, space_required);

    let to_delete = sizes.iter().filter(|s| *s >= &space_required).min();
    match to_delete {
        Some(val) => println!("To Delete: {}", val),
        None => println!("No directory satisfies the requirement."),
    }
